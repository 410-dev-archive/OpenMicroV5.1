...
void autonomous(void){
  double stTime, current_time;
  stTime = Brain.timer(vex::timeUnits::sec);
  while(1){
    current_time = Brain.timer(vex::timeUnits::sec) - stTime;
    if(current_time < 2){
      _left();
    }
    else if(2 < current_time && current_time < 4){
      _diag_leftfront();
    }
    else if(30 < current_time){
      break;
    }
  }
...
int main() {
  ...
  Competition.drivercontrol(autonomous2);
}
